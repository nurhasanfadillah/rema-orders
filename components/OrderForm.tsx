...
export const OrderForm: React.FC<Props> = ({ onBack, onSubmit, initialData, mode = 'create', onError, onOfflineQueued }) => {
  ...
  const handleSubmit = async () => {
    ...
    // --- NEW: OFFLINE MUTATION SUPPORT ---
    if (!navigator.onLine) {
      setUploadProgress('Menyimpan ke antrean offline...');
    ...
    try {
      ...
      // Upload All Files (Now Parallel & handled by uploadHelper)
      const uploadedPreviews = await uploadFiles(previewImages, 'sablon-files', mode, handleProgress);
      const uploadedDesigns = await uploadFiles(designFiles, 'sablon-files', mode, handleProgress);

      let uploadedReceipt = null;
      if (receiptFile) {
        const result = await uploadFiles([receiptFile], 'sablon-files', mode, handleProgress);
