...
interface Props {
    ...
    onUpdateDtfPrinted: (id: string, status: boolean) => void;
    onDelete: (id: string) => void;
    onEdit: (order: Order) => void;
    onCopy: (order: Order) => void;
}

export const OrderDetail: React.FC<Props> = ({ order, onBack, onUpdateStatus, onUpdateDtfPrinted, onDelete, onEdit, onCopy }) => {
    ...
    const [pendingDtfStatus, setPendingDtfStatus] = useState(false);
    ...
    const handleToggleDtf = (currentStatus: boolean) => {
        setPendingDtfStatus(!currentStatus);
        setShowDtfModal(true);
    };

    const confirmDtfToggle = () => {
        onUpdateDtfPrinted(order.id, pendingDtfStatus);
                ...
                <div className="flex justify-between items-end">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            {order.dtfPrinted && (
                                <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border flex items-center gap-1 bg-purple-500/20 text-purple-300 border-purple-500/30">
    ...
    return (
        <>
                    ...
                    <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 px-4 py-6 md:p-0">
                        ...
                        <div className="md:col-span-7 space-y-6">
                            ...
                            <div className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-6 shadow-xl relative overflow-hidden">
                                ...
                                {/* DTF Toggle */}
                                <div className="mt-4 p-4 bg-zinc-800/50 rounded-xl border border-zinc-700/50 flex items-center justify-between">
                                    <div>
                                        <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1">Cetak DTF</p>
                                        <p className="text-sm font-bold text-zinc-200">Tandai Selesai</p>
                                    </div>
                                    <button
                                        onClick={() => handleToggleDtf(order.dtfPrinted)}
                                        className={`relative inline-flex h-7 w-14 items-center rounded-full transition-colors shadow-inner ${order.dtfPrinted ? 'bg-purple-500' : 'bg-zinc-700'
                                            }`}
                                    >
                                        <span
                                            className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform shadow-md ${order.dtfPrinted ? 'translate-x-8' : 'translate-x-1'
                                                }`}
                                        />
                                    </button>
                                </div>
            ...
            {/* DTF Confirmation Modal */}
            <ConfirmationModal
                isOpen={showDtfModal}
                title={pendingDtfStatus ? "Selesai Cetak DTF?" : "Batalkan Status DTF?"}
                message={pendingDtfStatus
                    ? "Tandai pesanan ini telah selesai melalui proses cetak DTF?"
                    : "Batalkan status cetak DTF untuk pesanan ini?"}
                confirmText="Ya, Lanjutkan"
                cancelText="Batal"
                onConfirm={confirmDtfToggle}
                onCancel={() => setShowDtfModal(false)}
            />
        </>
    );
};
